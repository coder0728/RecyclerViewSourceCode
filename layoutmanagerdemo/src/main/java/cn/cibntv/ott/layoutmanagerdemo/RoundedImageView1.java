...
public class RoundedImageView1 extends android.support.v7.widget.AppCompatImageView {
	...
	public RoundedImageView1(Context context, AttributeSet attrs, int defStyle) {
		...
		if (mMutateBackground) {
			// when setBackground() is called by View constructor,
			// mMutateBackground is not loaded from the attribute,
			// so it's false by default, what doesn't allow to create the
			// RoundedDrawable. At this point, after load
			// mMutateBackground and updated BackgroundDrawable to
			// RoundedDrawable, the View's background drawable needs to
